...
def create_payment(*, payer, booking_id=None, course_id=None, payment_method='TEST', idempotency_key=None,
                   bitlibera_phone=None, bitlibera_otp=None):
    ...
    if booking:
        amount = _money(booking.tutor.hourly_rate * Decimal(booking.duration) / Decimal(60))
    ...
    provider_name = {'test': 'fake', 'bitcoin': 'blink', 'bitlibera': 'bitlibera'}.get(normalized_method)
    if provider_name is None:
        raise ValueError(
            f'Le moyen de paiement "{payment_method}" n’est pas encore connecté à un fournisseur réel.'
        )

    fee_rate = Decimal(str(getattr(settings, 'PAYMENT_PLATFORM_FEE_RATE', '0.05')))
    ...
    payment.bitlibera_phone = bitlibera_phone
    payment.bitlibera_otp = bitlibera_otp
    provider = get_payment_provider(payment.provider)
    response = provider.create_payment(payment)
    payment.provider_transaction_id = response['provider_transaction_id']
    payment.provider_payment_request = response.get('payment_request', '')
    payment.save(update_fields=['provider_transaction_id', 'provider_payment_request', 'updated_at'])
...
def process_provider_event(payment, event, provider_event_id=None, payload=None):
    ...
    if new_status == 'PAID':
        notify_payment_status(
            ...
            payment,
            'Paiement confirmé',
            f'Votre paiement pour {payment.course.title if payment.course else "votre réservation"} a bien été validé.',
        )
        if payment.payee_id and payment.payee_id != payment.payer_id:
            notify_payment_status(
                payment.payee,
                payment,
                'Nouveau paiement reçu',
                f'Vous avez reçu un paiement de {payment.amount} {payment.currency}.',
            )
