...
class CourseViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_staff:
            pass
        elif self.request.user.is_authenticated and self._is_mentor(self.request.user):
            queryset = queryset.filter(Q(status='PUBLISHED') | Q(is_published=True) | Q(teacher=self.request.user)).distinct()
        else:
            queryset = queryset.filter(Q(status='PUBLISHED') | Q(is_published=True)).distinct()
    ...
    @staticmethod
    def _is_mentor(user):
        return user.is_teacher or getattr(user, 'user_type', '') == 'TEACHER'
    ...
    def perform_create(self, serializer):
        if not self.request.user.is_authenticated or not self._is_mentor(self.request.user):
    ...
    def my_courses(self, request):
        """Liste des cours créés par le mentor connecté."""
        courses = self.get_queryset().filter(teacher=request.user)
...
class LessonViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if not self.request.user.is_teacher or course.teacher_id != self.request.user.id:
            raise PermissionDenied('Seul le mentor propriétaire peut ajouter une leçon.')
        serializer.save()

    def perform_update(self, serializer):
        course = serializer.validated_data.get('course', serializer.instance.course)
        if not self.request.user.is_staff and (not self.request.user.is_teacher or course.teacher_id != self.request.user.id):
            raise PermissionDenied('Vous ne pouvez modifier que les leçons de vos propres cours.')
        serializer.save()

    def perform_destroy(self, instance):
        if not self.request.user.is_staff and (not self.request.user.is_teacher or instance.course.teacher_id != self.request.user.id):
            raise PermissionDenied('Vous ne pouvez supprimer que les leçons de vos propres cours.')
        instance.delete()
...
class QuizViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        if not (self.request.user.is_teacher or getattr(self.request.user, 'user_type', '') == 'TEACHER'):
