...
class QuizSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Quiz
        fields = [
            'id', 'course', 'category', 'title', 'description', 'quiz_type', 'subject', 'school_level',
...
class QuizCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Quiz
        fields = [
            'id', 'course', 'category', 'chapter', 'title', 'description', 'quiz_type',
...
class LessonSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Lesson
        fields = [
            'id', 'course', 'chapter', 'title', 'content_type', 
    ...
    def validate(self, attrs):
        ...
        if chapter and course and chapter.course_id != course.id:
            raise serializers.ValidationError({'chapter': 'Ce chapitre ne fait pas partie de ce cours.'})
        if attrs.get('content_type', getattr(self.instance, 'content_type', 'TEXT')) == 'TEXT' and not attrs.get('text_content', getattr(self.instance, 'text_content', '')):
            raise serializers.ValidationError({'text_content': 'Le contenu texte est requis pour une leçon de type texte.'})
        if attrs.get('content_type', getattr(self.instance, 'content_type', 'TEXT')) == 'VIDEO' and not attrs.get('video_url', getattr(self.instance, 'video_url', '')):
            raise serializers.ValidationError({'video_url': 'Le lien vidéo est requis pour une leçon vidéo.'})
        return attrs
...
class ChapterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Chapter
        fields = ['id', 'course', 'title', 'summary', 'description', 'order', 'lessons', 'content_blocks']
