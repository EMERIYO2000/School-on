...
class AuthViewSet(viewsets.GenericViewSet):
    ...
    def become_mentor(self, request):
        ...
        return Response(TeacherApplicationSerializer(application, context={'request': request}).data, status=status.HTTP_201_CREATED)

    @action(detail=False, methods=['get'], url_path='my-mentor-application', permission_classes=[IsAuthenticated])
    def my_mentor_application(self, request):
        application = TeacherApplication.objects.filter(user=request.user).order_by('-created_at').first()
        return Response({
            'application': TeacherApplicationSerializer(application, context={'request': request}).data
    ...
    def list_mentor_applications(self, request):
        ...
        return Response(TeacherApplicationSerializer(queryset, many=True, context={'request': request}).data)
