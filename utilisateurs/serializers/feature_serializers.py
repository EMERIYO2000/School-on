...
class NearbyTutorSerializer(serializers.ModelSerializer):
    ...
    def get_reviews(self, obj):
        rows = BookingReview.objects.filter(booking__tutor=obj).select_related('booking__student')[:10]
        return [{
            'id': row.id,
            'rating': row.rating,
            'comment': row.comment,
            'author_name': row.booking.student.get_full_name() or row.booking.student.username,
    ...
    class Meta:
        ...
        fields = [
            ...
            'longitude', 
            'distance_km',
            'is_verified', 'subjects', 'rating', 'reviews'
        ]
